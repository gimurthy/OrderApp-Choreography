...
public class FulfillmentNotificationConsumer {

	private final Logger logger = LoggerFactory.getLogger(FulfillmentNotificationConsumer.class);

	@Autowired
	JSONParser parser;

	@KafkaListener(id = "FulfillmentNotificationConsumer", topics = { "${kafka.g-fulfillment-topic}" })
